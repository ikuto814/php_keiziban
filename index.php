...
if(!empty($_POST)){
  if($_POST['message']!==''){
    ...
    $message->bindParam(1, $_POST['message'], PDO::PARAM_STR);
    $message->bindParam(2, $member['id'], PDO::PARAM_INT);
...
if (isset($_REQUEST['res'])) {
  $response = $db->prepare('SELECT m.name, m.picture, p. * FROM members 
  m, posts p WHERE m.id=p.member_id AND p.id=?');
